...
def test_apply_work_sets_started_at():
    state = _state()
    state = apply_work(state, required_work_time=_time(10), dt=_time(1), t_now=_time(5))
...
def test_apply_work_accumulates_correctly():
    state = _state()
    state = apply_work(state, required_work_time=_time(10), dt=_time(3), t_now=_time(1))
    state = apply_work(state, required_work_time=_time(10), dt=_time(3), t_now=_time(2))
...
def test_apply_work_completes_task():
    state = _state()
    state = apply_work(state, required_work_time=_time(3), dt=_time(3), t_now=_time(7))
...
def test_apply_work_on_done_is_noop():
    state = _state()
    state = mark_done(state, _time(2))
    before_status = state.status
    before_work_done = state.work_done
    before_completed_at = state.completed_at
    result = apply_work(state, required_work_time=_time(10), dt=_time(1), t_now=_time(3))
    assert result.status == before_status
    assert result.work_done == before_work_done
    assert result.completed_at == before_completed_at


def test_apply_work_on_failed_is_noop():
    state = _state()
    state = mark_failed(state, _time(2))
    before_work_done = state.work_done
    before_completed_at = state.completed_at
    result = apply_work(state, required_work_time=_time(10), dt=_time(1), t_now=_time(3))
...
def test_started_at_not_overwritten():
    state = _state()
    state = apply_work(state, required_work_time=_time(10), dt=_time(1), t_now=_time(5))
    state = apply_work(state, required_work_time=_time(10), dt=_time(1), t_now=_time(6))
    assert state.started_at == _time(5)
...
def test_mark_done_sets_status_and_time():
    state = _state()
    state = mark_done(state, _time(42))
    assert state.status == TaskStatus.DONE
...
def test_mark_done_without_prior_work():
    ...
    state = _state()
    assert state.work_done == Time(0)
    state = mark_done(state, _time(10))
    assert state.status == TaskStatus.DONE
    assert state.work_done == Time(0)
...
def test_mark_failed_sets_status_and_time():
    state = _state()
    state = mark_failed(state, _time(99))
